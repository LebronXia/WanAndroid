package com.drakeet.years.widget
...
abstract class NewCustomLayout @JvmOverloads constructor(
  ...
  protected fun View.autoMeasure() {
    if (isGone) return
    measure(
      this.defaultWidthMeasureSpec(parentView = this@NewCustomLayout),
      this.defaultHeightMeasureSpec(parentView = this@NewCustomLayout)
    )
  }
  ...
  protected fun View.layout(x: Int, y: Int, fromRight: Boolean = false) {
    ...
    if (!fromRight) {
      ...
      layout(this@NewCustomLayout.measuredWidth - x - measuredWidth, y)
    }
  }
